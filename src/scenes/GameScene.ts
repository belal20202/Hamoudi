...
export class GameScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // No lives gate: the player can always retry a level immediately.
  ...
  private showResultOverlay(won: boolean, stars: 0 | 1 | 2 | 3 = 0): void {
    ...
    if (won) {
      ...
      this.add
        .text(cx, topY + 90, "حاول مرة أخرى، أنت قريب! 💪", UiStyle.body())
        .setOrigin(0.5)
        .setScrollFactor(0)
        .setDepth(2001);
    }
    ...
    // Both buttons below use the same premium textured button graphic as
    // every other menu in the app (shadow, gradient bevel, gold border) --
    // a plain text link with a flat background color read as an
    // afterthought next to it, which is what made "return to map" look
    // unfinished compared to the primary action.
    makeMenuButton(this, cx, btnY, retryLabel, () => {
      AudioService.click();
    ...
    makeMenuButton(this, cx, btnY + 52, "خريطة المراحل 🗺️", () => {
      AudioService.click();
